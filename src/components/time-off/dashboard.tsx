    ...
    <main className="appShell">
      <header className="topBar">
        ...
        <div className="topActions">
          {dashboard.state.toast ? (
            <div className={`toast toast-${dashboard.state.toast.tone}`}>{dashboard.state.toast.message}</div>
          ) : null}
      ...
      {dashboard.state.status === "loading" ? (
        ...
        <div className="dashboardGrid">
          <EmployeeView
            balances={dashboard.state.balances}
            requests={dashboard.state.requests}
            selectedLocationId={dashboard.state.selectedLocationId}
            form={dashboard.form}
            mode={dashboard.mode}
            busy={dashboard.isPending}
            onSelectLocation={dashboard.selectLocation}
            onFormChange={dashboard.setForm}
            onModeChange={dashboard.setMode}
            onSubmit={dashboard.submitRequest}
            onBonus={dashboard.applyBonus}
          />
          <ManagerView
            balances={dashboard.state.balances}
            requests={dashboard.state.requests}
            busy={dashboard.isPending}
            onDecision={dashboard.decide}
          />
        </div>
      )}
    </main>
